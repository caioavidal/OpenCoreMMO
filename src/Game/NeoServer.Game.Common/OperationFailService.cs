...
public static class OperationFailService
{
    ...
    public static void Display(IPlayer player, string message) => Display(player.CreatureId, message);
    public static void Display(IPlayer player, InvalidOperation operation)  => Display(player.CreatureId, operation);
}