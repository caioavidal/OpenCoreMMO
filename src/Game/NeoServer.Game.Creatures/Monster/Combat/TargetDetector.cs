...
public static class TargetDetector
{
    ...
    public static void UpdateTargets(Monster monster, IMapTool mapTool)
    {
        ...
        foreach (CombatTarget target in monster.Targets)
        {
            ...
            if (mapTool.SightClearChecker.Invoke(monster.Location, target.Creature.Location) && target.IsInRange(monster))
            {
    ...
    private static bool IsTargetUnreachable(Monster monster, CombatTarget target, IMapTool mapTool, out Direction[] directions)
    {
