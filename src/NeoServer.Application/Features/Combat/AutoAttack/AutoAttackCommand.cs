...
public class AutoAttackCommandHandler(AttackLibrary attackLibrary, PlayerAttackSelector playerAttackSelector, GameConfiguration gameConfiguration) : ICommandHandler<AutoAttackCommand, Result>
{
    public ValueTask<Result> Handle(AutoAttackCommand command, CancellationToken cancellationToken)
    {
        ...
        var attackParameters = playerAttackSelector.Select(aggressor as IPlayer);
