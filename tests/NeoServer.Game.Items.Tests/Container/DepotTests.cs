...
public class DepotTests
{
    ...
    [Fact]
    public void Depot_is_saved_when_player_closes_it()
    {
        //arrange
        var player1 = PlayerTestDataBuilder.Build(id: 1);
        ...
        var repositoryMock = new Mock<IPlayerDepotItemRepository>();

        player1.Containers.OnClosedDepot += (player, _, container) =>
            new PlayerClosedDepotEventHandler(repositoryMock.Object).Execute(player, _, (IDepot)container);

        //act
        player1.Containers.CloseContainer(0);

        //assert
        repositoryMock.Verify(x => x.DeleteAll(player1.Id), Times.Once);
        repositoryMock.Verify(x => x.Save(player1, depot), Times.Once);
    }

    [Fact]
    public void Depot_is_saved_when_player_closes_a_inner_container()
    {
        //arrange
        var player1 = PlayerTestDataBuilder.Build(id: 1);
        ...
        player1.Containers.OnClosedDepot += (player, _, container) =>
            new PlayerClosedDepotEventHandler(repositoryMock.Object).Execute(player, _, container as Depot);
